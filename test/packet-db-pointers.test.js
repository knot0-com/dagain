...
test("run: injects DB pointer env vars into runner", async () => {
  ...
  assert.equal(envRecord.CHOREO_DB, path.join(tmpDir, ".choreo", "state.sqlite"));
  assert.equal(envRecord.CHOREO_NODE_ID, "plan-000");
  assert.equal(envRecord.CHOREO_ARTIFACTS_DIR, path.join(tmpDir, ".choreo", "artifacts"));
  assert.equal(envRecord.CHOREO_CHECKPOINTS_DIR, path.join(tmpDir, ".choreo", "checkpoints"));
  assert.equal(envRecord.CHOREO_RUNS_DIR, path.join(tmpDir, ".choreo", "runs"));
